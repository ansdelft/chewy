...
describe Chewy::OpensearchClient do
  describe 'payload inspection' do
    let(:filter) { instance_double('Proc') }
    let!(:filter_previous_value) { Chewy.before_os_request_filter }
    ...
    before do
      ...
      Chewy.before_os_request_filter = filter
    end

    after do
      Chewy.before_os_request_filter = filter_previous_value
    end
